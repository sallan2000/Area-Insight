...
export default function Compare() {
  ...
  // True when exactly one postcode is Scottish (safety excluded from its total)
  const isMixed = !!(report1 && report2 &&
    ((raw1?.crimeDataUnavailable && !raw2?.crimeDataUnavailable) ||
     (!raw1?.crimeDataUnavailable && raw2?.crimeDataUnavailable)));

  // Three-factor score using only the shared categories (transport/schools/amenities)
  // at redistributed weights — the only fair like-for-like comparison across both countries
  const threeFactorTotal = (s: any) =>
    Math.round((s.transport * (25 / 65)) + (s.amenities * (20 / 65)) + (s.schools * (20 / 65)));
        ...
        {report1 && report2 && (
          <div ref={comparisonRef} className="space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {isMixed && (
                <div className="md:col-span-2 flex gap-4 p-4 bg-amber-50 border border-amber-200 rounded-2xl" data-testid="mixed-comparison-warning">
                  <div className="shrink-0 mt-0.5 text-amber-500">
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z" /></svg>
                  </div>
                  <div>
                    <p className="text-sm font-bold text-amber-800 mb-1">Scores are not directly comparable</p>
                    <p className="text-xs text-amber-700">
                      One postcode is in Scotland, where Police Scotland does not publish street-level crime data.
                      The Scottish score is based on <span className="font-semibold">3 factors</span> (Transport, Schools, Amenities),
                      while the English score uses <span className="font-semibold">4 factors</span> including Safety.
                      See the <span className="font-semibold">Like-for-Like Comparison</span> below for a fair side-by-side on shared categories only.
                    </p>
                  </div>
                </div>
              )}
              ...
              {isMixed && (
                <div className="md:col-span-2 bg-white rounded-2xl p-8 border shadow-sm" data-testid="like-for-like-comparison">
                  <div className="flex items-center gap-2 mb-2">
                    <h2 className="text-xl font-bold text-center flex-1">Like-for-Like Comparison</h2>
                  </div>
                  <p className="text-xs text-muted-foreground text-center mb-8">
                    Transport, Schools &amp; Amenities only — the three categories available for both postcodes, weighted equally.
                  </p>
                  <div className="flex items-center justify-between gap-8">
                    <div className="flex-1 text-center space-y-3">
                      <p className="text-lg font-bold">{report1.postcode}</p>
                      <div className={`text-5xl font-black ${getWinner(threeFactorTotal(report1Scores), threeFactorTotal(report2Scores)) === 1 ? 'text-primary' : 'text-muted-foreground'}`}>
                        {threeFactorTotal(report1Scores)}
                      </div>
                      {getWinner(threeFactorTotal(report1Scores), threeFactorTotal(report2Scores)) === 1 && (
                    ...
                    <div className="flex-1 text-center space-y-3">
                      <p className="text-lg font-bold">{report2.postcode}</p>
                      <div className={`text-5xl font-black ${getWinner(threeFactorTotal(report1Scores), threeFactorTotal(report2Scores)) === 2 ? 'text-primary' : 'text-muted-foreground'}`}>
                        {threeFactorTotal(report2Scores)}
                      </div>
                      {getWinner(threeFactorTotal(report1Scores), threeFactorTotal(report2Scores)) === 2 && (
                        <Badge className="bg-emerald-500">WINNER</Badge>
                      )}
                    </div>
                  </div>
                  <div className="mt-8 grid grid-cols-3 gap-4 pt-6 border-t border-border">
                    {[
                      { label: 'Transport', icon: Bus, key: 'transport' },
                      { label: 'Schools', icon: GraduationCap, key: 'schools' },
                      { label: 'Amenities', icon: Store, key: 'amenities' },
                    ].map(({ label, icon: Icon, key }) => {
                      const v1 = report1Scores[key];
                      const v2 = report2Scores[key];
                      const win = getWinner(v1, v2);
                      return (
                        <div key={key} className="text-center space-y-2">
                          ...
                          <div className="flex items-center justify-around">
                            <span className={`text-2xl font-bold ${win === 1 ? 'text-emerald-600' : win === 2 ? 'text-red-500' : 'text-foreground'}`}>{v1}</span>
                            <span className="text-xs text-muted-foreground/40 font-bold">vs</span>
                            <span className={`text-2xl font-bold ${win === 2 ? 'text-emerald-600' : win === 1 ? 'text-red-500' : 'text-foreground'}`}>{v2}</span>
                          </div>
                        </div>
                      );
                    })}
                  ...
                  <tbody className="divide-y">
                    {[
                      { name: 'Safety', key: 'safety', icon: Shield, type: 'score',
                        override1: raw1?.crimeDataUnavailable ? 'N/A' : null,
                        override2: raw2?.crimeDataUnavailable ? 'N/A' : null },
                      ...
                      const raw1Val = cat.type === 'score' ? report1Scores[cat.key] : cat.getValue?.(raw1);
                      const raw2Val = cat.type === 'score' ? report2Scores[cat.key] : cat.getValue?.(raw2);
                      const val1 = cat.override1 ?? raw1Val;
                      const val2 = cat.override2 ?? raw2Val;
                      // Only colour-code the winner when both values are comparable numbers
                      const bothNumeric = typeof val1 === 'number' && typeof val2 === 'number';
                      const win = (cat.type === 'score' && bothNumeric) ? getWinner(val1, val2) : 0;
                      ...
                      return (
                        <tr key={cat.name} className="hover:bg-gray-50/50 transition-colors">
                          ...
                          <td className="px-6 py-4 text-center">
                            <span className={`text-lg font-bold ${val1 === 'N/A' ? 'text-muted-foreground' : win === 1 ? 'text-emerald-600' : win === 2 ? 'text-red-600' : ''}`}>{val1}</span>
                          </td>
                          <td className="px-6 py-4 text-center">
                            <span className={`text-lg font-bold ${val2 === 'N/A' ? 'text-muted-foreground' : win === 2 ? 'text-emerald-600' : win === 1 ? 'text-red-600' : ''}`}>{val2}</span>
                          </td>
                        </tr>
                      );
                    })}
