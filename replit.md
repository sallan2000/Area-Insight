...
- **GOV EPC Register API**: Per-postcode Energy Performance Certificate band + estimated heating/energy cost. Free, Open Government Licence, but requires a free API key (register at epc.opendatacommunities.org) supplied via the `EPC_API_KEY` env var. Endpoint: `https://epc.opendatacommunities.org/api/v1/domestic/search?postcode=...` with `Authorization` header. Field: `rawMetrics.epc` (degrades to "register free key" when no key / "not available" on error). Does NOT affect liveability score.
...
- **`server/data/census-demographics.json`**: ONS Census 2021 LSOA demographics (population, % under 18, % 65+, tenure mix, % no car), keyed by LSOA21. Built from a joined ONS LSOA CSV by `npm run sync:census-demographics` (free, OGL). Field: `rawMetrics.demographics` (degrades to "run sync" when absent). All-UK; lookup key is `codes.lsoa21` from postcodes.io.
