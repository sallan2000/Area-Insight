      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
          <div style={{ background: "#f0fdf4", border: "1px solid #bbf7d0", borderRadius: 8, padding: "10px 14px", fontSize: 12, color: "#166534" }}>
            <strong>{raw.green?.count || 0}</strong> green spaces within 1.5 km
            {raw.green?.nearestDistance != null ? ` (nearest ${raw.green.nearestDistance.toFixed(2)} km)` : ""}.
          </div>
          <div style={{ background: "#eff6ff", border: "1px solid #bfdbfe", borderRadius: 8, padding: "10px 14px", fontSize: 12, color: "#1e40af" }}>
            <strong>{raw.health?.count || 0}</strong> GPs / clinics / hospitals / dentists nearby
            {raw.health?.nearestDistance != null ? ` (nearest ${raw.health.nearestDistance.toFixed(2)} km)` : ""}.
          </div>
        </div>
        <p style={{ fontSize: 10, color: "#9ca3af", marginTop: 8 }}>Green space &amp; health access from OpenStreetMap — indicative of provision, not NHS service availability.</p>
      </div>
